...
namespace BuildReportTool
{
	public static class Util
	{
		...
		public static string GetTextureDataDefaultFilename(string projectName, string buildType,
			System.DateTime timeGot)
		{
			return string.Format("TextureData-{0}-{1}-{2}.xml", projectName, buildType, timeGot.ToString("yyyyMMMdd-HHmmss"));
		}
		...
		public static string SerializeBuildInfoAtFolder(BuildReportTool.BuildInfo buildInfo, string folderPathToSaveTo)
		{
		...
		public static void SerializeBuildInfo(BuildReportTool.BuildInfo buildInfo, string xmlFileFullPathToSaveTo)
		{
			var x = new System.Xml.Serialization.XmlSerializer(typeof(BuildReportTool.BuildInfo));
			System.IO.TextWriter writer = new System.IO.StreamWriter(xmlFileFullPathToSaveTo);
			x.Serialize(writer, buildInfo);
			writer.Close();

			buildInfo.SetSavedPath(xmlFileFullPathToSaveTo);

			Debug.LogFormat("Build Report Tool: Saved build info at \"{0}\"", buildInfo.SavedPath);
		}

		// ---------------------------------

		public static BuildReportTool.AssetDependencies OpenSerializedAssetDependencies(string serializedAssetDependenciesFilePath)
		{
			BuildReportTool.AssetDependencies ret = null;

			var x = new System.Xml.Serialization.XmlSerializer(typeof(BuildReportTool.AssetDependencies));

			try
			{
				// no corrections in the xml file
				// proceed to open the file normally
				using (var fs = new System.IO.FileStream(serializedAssetDependenciesFilePath, System.IO.FileMode.Open))
				{
					System.Xml.XmlReader reader = new System.Xml.XmlTextReader(fs);
					ret = (BuildReportTool.AssetDependencies) x.Deserialize(reader);
					fs.Close();
				}
			}
			catch (Exception e)
		...
		public static BuildReportTool.TextureData OpenSerializedTextureData(string serializedTextureDataFilePath)
		{
			BuildReportTool.TextureData ret = null;

			var x = new System.Xml.Serialization.XmlSerializer(typeof(BuildReportTool.TextureData));

			try
			{
				// no corrections in the xml file
				// proceed to open the file normally
				using (var fs = new System.IO.FileStream(serializedTextureDataFilePath, System.IO.FileMode.Open))
				{
					System.Xml.XmlReader reader = new System.Xml.XmlTextReader(fs);
					ret = (BuildReportTool.TextureData) x.Deserialize(reader);
					fs.Close();
				}
			}
			catch (Exception e)
			...
			{
				ret.OnAfterLoad();
				ret.SetSavedPath(serializedTextureDataFilePath);
			}
		...
		public static string SerializeTextureDataAtFolder(BuildReportTool.TextureData textureData,
			string folderPathToSaveTo)
		{
			...
			//Debug.Log("built folder");
			filePath = string.Format("{0}{1}", filePath, textureData.GetDefaultFilename());
			//Debug.Log("built filepath " + filePath);

			SerializeTextureData(textureData, filePath);

			return filePath;
		}

		public static void SerializeTextureData(BuildReportTool.TextureData textureData, string xmlFileFullPathToSaveTo)
		{
			textureData.OnBeforeSave();

			System.Xml.Serialization.XmlSerializer x = new System.Xml.Serialization.XmlSerializer(typeof(BuildReportTool.TextureData));
			System.IO.TextWriter writer = new System.IO.StreamWriter(xmlFileFullPathToSaveTo);
			x.Serialize(writer, textureData);
			writer.Close();

			textureData.SetSavedPath(xmlFileFullPathToSaveTo);

			Debug.LogFormat("Build Report Tool: Saved texture data at \"{0}\"", textureData.SavedPath);
		}

		public static string SerializeAssetDependenciesAtFolder(BuildReportTool.AssetDependencies assetDependencies,
			string folderPathToSaveTo)
		{
			string filePath = folderPathToSaveTo;
			if (!string.IsNullOrEmpty(folderPathToSaveTo))
		...
		public static void SerializeAssetDependencies(BuildReportTool.AssetDependencies assetDependencies, string xmlFileFullPathToSaveTo)
		{
			assetDependencies.OnBeforeSave();

			System.Xml.Serialization.XmlSerializer x = new System.Xml.Serialization.XmlSerializer(typeof(BuildReportTool.AssetDependencies));
			System.IO.TextWriter writer = new System.IO.StreamWriter(xmlFileFullPathToSaveTo);
			...
			Debug.LogFormat("Build Report Tool: Saved asset dependencies at \"{0}\"", assetDependencies.SavedPath);
		}
