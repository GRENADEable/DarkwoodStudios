...
public class BRT_BuildReportWindow : EditorWindow
{
	...
	static void GetFromNative(GUIStyle ownStyle, GUIStyle nativeStyle, out GUIStyle styleToAssign)
	{
		if (nativeStyle == null)
		{
			styleToAssign = null;
	...
	{
		GoToOverviewScreen();
		BuildReportTool.ReportGenerator.RefreshData(ref _buildInfo, ref _assetDependencies, ref _textureData);
	}
	...
	{
		BuildReportTool.ReportGenerator.OnFinishedGetValues(_buildInfo, _assetDependencies, _textureData);
	...
	void _OpenBuildInfo(string filepath)
	{
		...
		{
			var loadedTextureData = BuildReportTool.Util.OpenSerializedTextureData(textureDataFilePath);
	...
	void DrawTopRowButtons()
	{
		    ...
		    BuildReportTool.Util.BuildInfoHasContents(_buildInfo))
		{
			...
			if (!string.IsNullOrEmpty(filepath))
			{
				...
				{
					var textureDataFilePath = BuildReportTool.Util.GetTextureDataFilenameFromBuildInfo(filepath);
					BuildReportTool.Util.SerializeTextureData(_textureData, textureDataFilePath);
				}
			}
		}
	...
	void OnGUI()
	{
		...
		if (_buildInfoHasNoContentsToDisplay)
		{
			...
			{
				GUILayout.Space(40);
				_optionsScreen.DrawGUI(position, _buildInfo, _assetDependencies, _textureData, out requestRepaint);
			}
			else if (IsInHelpCategory)
			{
				GUILayout.Space(40);
				_helpScreen.DrawGUI(position, _buildInfo, _assetDependencies, _textureData, out requestRepaint);
			}
		...
		{
			_overviewScreen.DrawGUI(position, _buildInfo, _assetDependencies, _textureData, out requestRepaintOnTabs);
		}
		else if (IsInBuildSettingsCategory)
		{
			_buildSettingsScreen.DrawGUI(position, _buildInfo, _assetDependencies, _textureData, out requestRepaintOnTabs);
		}
		else if (IsInSizeStatsCategory)
		{
			_sizeStatsScreen.DrawGUI(position, _buildInfo, _assetDependencies, _textureData, out requestRepaintOnTabs);
		}
		else if (IsInUsedAssetsCategory)
		{
			_usedAssetsScreen.DrawGUI(position, _buildInfo, _assetDependencies, _textureData, out requestRepaintOnTabs);
		}
		else if (IsInUnusedAssetsCategory)
		{
			_unusedAssetsScreen.DrawGUI(position, _buildInfo, _assetDependencies, _textureData, out requestRepaintOnTabs);
		}
		else if (IsInOptionsCategory)
		{
			_optionsScreen.DrawGUI(position, _buildInfo, _assetDependencies, _textureData, out requestRepaintOnTabs);
		}
		else if (IsInHelpCategory)
		{
			_helpScreen.DrawGUI(position, _buildInfo, _assetDependencies, _textureData, out requestRepaintOnTabs);
		}
